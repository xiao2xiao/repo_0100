...
- 项目描述
	...
	> **_`其它`_**：使用eureka server作为服务注册中心，ribbon和feigin进行服务之间的调用，hystrix进行服务的降级，sleuth进行全链路的追踪、zipkin+http进行日志的实时追踪并关注请求链路中各个阶段的时间延迟、并使用ELK（Elasticsearch , Logstash, Kibana）架构对日志进行收集和检索，以及使用Grok Debugger编写日志的过滤。
